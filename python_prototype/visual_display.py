...
def update(frame):
    ...
    for _ in range(steps_per_frame):
        ...
        current_state_info = RK4_new_state(current_time, current_state, time_interval, mfr, m_rocket_dry, m_fuel_max, v_wind)
    ...
    up_eci = current_state[0:3] / np.linalg.norm(current_state[0:3]) # vector that we fix to take the rocket's orientation as reference to
    ori = current_state[6:10]
    nose_ori_bf = np.array([1.0, 0.0, 0.0])
    nose_ori_eci = b2eci(nose_ori_bf, ori)
    north_pole_eci = np.array([0.0, 0.0, 1.0])
    ...
    R_eci_to_local = np.vstack([east_eci, north_local, up_eci])

    up_local = R_eci_to_local @ up_eci       
    nose_local = R_eci_to_local @ nose_ori_eci
