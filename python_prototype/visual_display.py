...
dry_CoG_launch = start_dist(earth_rad_launch)

initial_state = np.array([
    dry_CoG_launch, 0.0, 0.0,   # 0:2 - Position of rocket's dry CoM (ECIF meters)
...
def update(frame):
    ...
    current_state = RK4_new_state(current_time, current_state, time_interval, mfr, m_rocket_dry, m_fuel_max)

    # Convert new ECI position to ECEF
    current_rocket_r = eci_to_ecef(current_state[0:3], current_time)
    
    # check if rocket is in earth, if so stop!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
