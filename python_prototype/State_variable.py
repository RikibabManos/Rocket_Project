...
df = pd.read_csv(
    r"C:\Codes\Rocket_project\coeff_cvs\gemini_saturn_v_CoP_lookup.csv",
    index_col = 0
    ) 
...
dry_CoG = np.array( [41.0, 0, 0] ) # approcimate CoG distance vector from bottom of dry rocket, in meters, in rocket bf
...
def get_MoI(dry_mass, fuel_mass, diameter, rocket_height, fuel_height, total_CoG, dry_CoG, fuel_CoG): # note CoGs must be vector taken from the bottom of the rocket, in the bf
    ...
    MoI_tensor_dry_shifted = MoI_tensor_dry + ( dry_mass * ( ( dot_prod(total_to_dry_CoG, total_to_dry_CoG) * I ) - np.outer(total_to_dry_CoG, total_to_dry_CoG)) )
    MoI_tensor_fuel_shifted = MoI_tensor_fuel + ( fuel_mass * ( ( dot_prod(total_to_fuel_CoG, total_to_fuel_CoG) * I ) - np.outer(total_to_fuel_CoG, total_to_fuel_CoG)) )
...
def get_derivatives(t, state, mfr, dry_mass, fuel_mass_max):
    ...
    height_of_fuel = get_CoG_fuel_bf(fuel_tank_height, bottom_to_tank, fuel_mass, fuel_mass_max)[0]
    ...
    MoI = get_MoI(dry_mass, fuel_mass, d, total_rocket_height, height_of_fuel, total_CoG, dry_CoG, CoG_fuel_bf) # in rocket bf
    net_torque_bf = lift_torque + drag_torque
...
def RK4_new_state(t, state, dt, mfr, dry_mass, fuel_max): 

    # check how much fuel is left (in kg)
    fuel_mass_remaining = state[13] - dry_mass

    print(f" ------------------------------------------------------------------------------ \n Time = {t}s, Dry mass = {dry_mass}kg, Fuel mass = {fuel_mass_remaining}kg, Total mass = {state[13]}kg, Altitude = {get_true_altitude(state[0:3]):.1f}m") # mission control display
    ...
    k1 = get_derivatives(t, state, mfr_adjusted, dry_mass, fuel_max)
    k2 = get_derivatives( ( t + dt / 2 ) , ( state + ( (dt * k1) / 2) ), mfr_adjusted, dry_mass, fuel_max )
    k3 = get_derivatives( ( t + dt / 2 ) , ( state + ( (dt * k2) / 2) ), mfr_adjusted, dry_mass, fuel_max )
    k4 = get_derivatives( t + dt , ( state +  (dt * k3) ), mfr_adjusted, dry_mass, fuel_max )
